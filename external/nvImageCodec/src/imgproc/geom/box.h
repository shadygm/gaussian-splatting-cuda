...
namespace nvimgcodec {
    ...
    struct Box {
        ...
        constexpr NVIMGCODEC_HOST_DEV corner_t centroid() const {
            return 0.5 * (hi + lo);
        }
         ...
         */
        constexpr NVIMGCODEC_HOST_DEV bool contains(const corner_t& point) const {
         ...
         */
        constexpr NVIMGCODEC_HOST_DEV bool contains(const Box& other) const {
            for (int i = 0; i < ndims; i++) {
         ...
         */
        constexpr NVIMGCODEC_HOST_DEV bool overlaps(const Box& other) const {
            for (int i = 0; i < ndims; i++) {
         ...
         */
        constexpr NVIMGCODEC_HOST_DEV bool empty() const {
     ...
     */
    template <int ndims, typename CoordinateType>
    constexpr NVIMGCODEC_HOST_DEV CoordinateType volume(const Box<ndims, CoordinateType>& box) {
        return nvimgcodec::volume(box.extent());
    }
     ...
     */
    template <int ndims, typename CoordinateType>
    constexpr NVIMGCODEC_HOST_DEV Box<ndims, CoordinateType> intersection(
    ...
    template <int ndims, typename CoordinateType>
    constexpr NVIMGCODEC_HOST_DEV CoordinateType intersection_over_union(
        const Box<ndims, CoordinateType>& lhs, const Box<ndims, CoordinateType>& rhs) {
    ...
    template <int ndims, typename CoordinateType>
    constexpr NVIMGCODEC_HOST_DEV bool
